defmodule IgIntranetWeb.CoreComponents do
  @moduledoc """
  ...
  def switch_locale_path(assigns) do
    ~H"""
    <% class =
      (@active && "inline-block px-4 py-2 border-b-2 border-indigo-600 text-indigo-600 font-semibold") ||
        "inline-block px-4 py-2 text-gray-700 hover:text-black" %>

    <a href={"/?locale=#{@locale}"} class={class}>{@language}</a>
    """
  end
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(IgIntranetWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(IgIntranetWeb.Gettext, "errors", msg, opts)
    end
  end
